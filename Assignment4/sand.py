...
This function generates an interface in which by clicking,
the user simulates the flow of sand. The user can also create
obstructions, defined as rocks, which inhibit the flow of the sand.
This simulation also incorporates Brownian physics motion to try and
capture the true movement of sand.
"""
...
def do_move(grid, x1, y1, x2, y2):
    ...
    # temp is created to allow for the reversal of the indexes
    temp = grid[y1][x1]
...
def check_move(grid, x1, y1, x2, y2):
    ...
    # checking if destination is outside boundaries or that its not empty
    if y2 >= len(grid) or x2 >= len(grid[y2]) or grid[y2][x2] is not None:
        return False

    # down right blocked condition
    if x2 == x1 + 1 and y2 == y1 + 1:
        if grid[y1][x1 + 1] is not None:
            return False
...
def do_gravity(grid, x, y):
    ...
    # If sand is not at location (x,y)
    if grid[y][x] != 's':
        return grid
...
def do_brownian(grid, x, y, brownian):
    ...
    if grid[y][x] == 's':
        ...
        if num < brownian:
            ...
            # moving sand one cell to the left
            if coin == 0:
                if check_move(grid, x, y, x - 1, y) is True:
                    grid = do_move(grid, x, y, x - 1, y)
                    return grid

            # moving sand one cell to the right
            else:
                if check_move(grid, x, y, x + 1, y) is True:
                    grid = do_move(grid, x, y, x + 1, y)
                    return grid
...
def do_whole_grid(grid, brownian):
    ...
    for i in range(len(grid[0])):
        for j in reversed(range(len(grid))):
            grid = do_gravity(grid, i, j)
            do_brownian(grid, i, j, brownian)
