...
This program is an extremely simple version of a search engine, like Google. It processes
...
def create_index(filenames, index, file_titles):
    ...
    for i in range(len(filenames)):
        with open(filenames[i]) as temp_file:
            # Processing title into file_titles dictionary
            line1 = temp_file.readline()
            line1 = line1.strip()
            file_titles[filenames[i]] = line1

            # Adding words in title to index dictionary
            parts = line1.split()
            for elem in parts:
                temp_elem = elem.strip(string.punctuation)
                temp_elem = temp_elem.lower()

                if temp_elem != '':
                    # Checking that word is not in index dictionary so it can be added
                    if temp_elem not in index:
                        index[temp_elem] = []

                    # Condition so that each key has distinct values (i.e filename does not appear twice)
                    if filenames[i] not in index[temp_elem]:
                        index[temp_elem].append(filenames[i])

            # Adding words from each line to index dictionary
...
def search(index, query):
    ...
    # Converting the user inputted query into a list
    query = query.strip()
    query = query.split()

    # Creating the list that contains all values from the index dictionary whose key match the query
    for word in query:
        if word in index:
            temp_lst1.append(index[word])

    # Condition to check that user input is in the index dictionary
    if len(temp_lst1) != len(query):
        return []

    # Finding the 'best' file for the user's query
    for word in query:
        posting_list = index[word]

        for i in range(1, len(temp_lst1)):
            posting_list = common(posting_list, temp_lst1[i])
        return posting_list
